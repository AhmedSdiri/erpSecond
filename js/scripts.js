...
$(document).ready(function() {
	...
	function updateTotals(elem) {
		...
		var istva = $('[name="invoice_product_tav[]"]', tr).val().indexOf('%') > -1;
	    var istvat = $.trim($('[name="invoice_product_tav[]"]', tr).val().replace('%', ''));
		...
		$('.calculate-sub', tr).val(subtotal.toFixed(2));
			if(istvat && $.isNumeric(istvat) && istvat !== 0)
			{
				if(istva){
					subtotal = subtotal + ((parseFloat(istvat) / 100) * subtotal);
				}
				else {
                subtotal = subtotal + parseFloat(istvat);
            		}
			}else 
			{
            	$('[name="invoice_product_tav[]"]', tr).val('');
        	}
		$('.calculate-sub', tr).val(subtotal.toFixed(2)); 
	}
	...
	function actionAddProduct() {
		...
		if (errorCounter > 0) {
		    $("#response").removeClass("alert-success").addClass("alert-warning").fadeIn();
		    $("#response .message").html("<strong>Erreur</strong>: Champs requis!");
		    $("html, body").animate({ scrollTop: $('#response').offset().top }, 1000);
		} else {
		...
		if (errorCounter > 0) {
			...
			$.ajax({
				...
				success: function(data){
					...
					$("#response").removeClass("alert-warning").addClass("alert-success").fadeIn();
					$("html, body").animate({ scrollTop: $('#response').offset().top }, 1000);
					$("#create_customer").before().html("<a href='/customer-add.php' class='btn btn-primary'>Nouveau client</a>");
	...
	function actionLogin() {
		...
		if (errorCounter > 0) {
		    ...
		    $("#response").removeClass("alert-success").addClass("alert-warning").fadeIn();
		    $("#response .message").html("<strong>Erreur</strong>: Vérifier les champs !");
		    $("html, body").animate({ scrollTop: $('#response').offset().top }, 1000);

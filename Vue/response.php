...
if ($action == 'email_invoice'){
	...
	if (empty($custom_email)){
		...
		} else if($invoice_type == 'Bon de commande'){
			$mail->MsgHTML(EMAIL_BODY_QUOTE);
		} else if($invoice_type == 'Facture'){
	...
	if(!$mail->Send()) {
	    ...
	    echo json_encode(array(
	    	...
	    	'message' => 'Email non envoyé vérifiez de .<pre>'.$mail->ErrorInfo.'</pre>'
	    ));
	} else {
	   echo json_encode(array(
			'status' => 'Success',
...
if ($action == 'download_csv'){
    ...
    if ($result_column_data = mysqli_query($mysqli, $query_table_columns_data)) {
		echo json_encode(array(
			'status' => 'Success',
			'message'=> 'La liste des devis est génerée <a href="/downloads/'.$file_name.'">clickez ici</a>.'
		));
	    ...
	    echo json_encode(array(
	    	'status' => 'Error',
	    	//'message'=> 'There has been an error, please try again.'
	    	'message' => 'Erreur.<pre>'.$mysqli->error.'</pre><pre>'.$query.'</pre>'
	    ));
	}
...
if ($action == 'create_customer'){
	...
	if($stmt->execute()){
		...
		echo json_encode(array(
			'status' => 'Error',
			'message' => 'Données non enregistrés.'
			// debug
...
if ($action == 'create_invoice'){
	...
	if($mysqli -> multi_query($query)){
		...
		//Set to
		$invoice->setTo(array($customer_name,$customer_address_1,$customer_address_2,$customer_town,$customer_county,$customer_postcode,"Telephone: ".$customer_phone));
		//Ship to
		...
		if(!empty($invoice_discount)) {
			$invoice->addTotal("Réduction",$invoice_discount);
		}
		if(!empty($invoice_shipping)) {
			$invoice->addTotal("Livraison",$invoice_shipping);
		}
		if(ENABLE_VAT == true) {
			$invoice->addTotal("TAX/TVA ".VAT_RATE."%",$invoice_vat);
		}
		$invoice->addTotal("Grand total",$invoice_total,true);
		//Add Badge
		...
		//Add Title
		$invoice->addTitle("Informations de paiment");
		//Add Paragraph
		...
		echo json_encode(array(
			'status' => 'Error',
			'message' => 'Erreur lors de création du document.'
			// debug
			//'message' => 'There has been an error, please try again.<pre>'.$mysqli->error.'</pre><pre>'.$query.'</pre>'
		));
	}
...
if($action == 'add_product') {
	...
	/* Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob */
	$stmt->bind_param('sssss',$product_name,$product_desc,$product_tva,$product_price);
	...
	if($stmt->execute()){
	    ...
	    echo json_encode(array(
	    	'status' => 'Erreur',
	    	//'message'=> 'There has been an error, please try again.'
	    	'message' => 'Revérifier les données.<pre>'.$mysqli->error.'</pre><pre>'.$query.'</pre>'
	    ));
	}
...
if($action == 'add_user') {
	...
	/* Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob */
	$stmt->bind_param('ssss',$user_name,$user_username,$user_email,$user_phone,$user_password);
